...
> [!WARNING]
> このbranchは、iOSで録画の画質切替を繰り返した際に発生する`InvalidStateError`の診断専用です。MSE操作名、`sourceopen` / `sourceclose`の回数と直近イベントからの時間、失敗直前のvideoと`<source>`の接続状態をエラーへ追加しますが、不具合を修正するものではなく、branch全体の取り込みは想定していません。
