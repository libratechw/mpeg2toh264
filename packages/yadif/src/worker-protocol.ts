...
export type WorkerCommand =
  ...
  | { type: "settings"; options: WorkerRenderingOptions }
...
export type WorkerNotification =
   ...
   */
  | { type: "diagnostic"; meta: DiagnosticPictureMeta }
  /** queueを通過して実際にcanvasへ描かれた加工出力。frameはtransferable。 */
  | { type: "presented"; frame: VideoFrame; meta: PresentedPictureMeta }
  /** 旧表示 queue の間引き前に取り出した加工画素。frameはtransferable。 */
  | { type: "queued"; frame: VideoFrame; meta: QueuedPictureMeta }
  /** sink へ直接書込んだ queued frame の meta（画素は含まない）。 */
  | { type: "queuedMeta"; meta: QueuedPictureMeta }
  /** 旧表示 queue の期限判定。画素・物理表示は含まない。 */
  | { type: "presentationQueue"; meta: PresentationQueueMeta };
