...
`;class y{static CYCLE=5;static COMB_THRESHOLD=9;static COMBED_PIXEL_LIMIT=80;static DECIMATE_BLOCK=32;static DUPLICATE_PERCENT=1.1;#n;#i;#e;#s=0;#x=null;#a=[];#T=null;#O=1/0;#z=1/0;constructor(e,t){this.#n=e,this.#i=t,this.#e=255*y.DECIMATE_BLOCK**2*y.DUPLICATE_PERCENT/100}fieldMatch(e,t,i,s,r=y.COMBED_PIXEL_LIMIT){const n=s?1:0,u={p:e,c:t,n:i};let o=this.#w("c","p",n,u);const c=new Map,h=d=>{const E=c.get(d);if(E!==void 0)return E;const g=y.#L(this.weave(e,t,i,d,s),this.#n,this.#i);return c.set(d,g),g},v=h(o),l=h("n");(l*3<v||l*2<v&&v>r)&&Math.abs(l-v)>=30&&l<r&&(o="n");const a=h(o),p=a>=r;return p&&(o="c"),{match:o,combScore:a,isCombed:p,luma:this.weave(e,t,i,o,s)}}decimate(e){const t=this.#s,i=this.#T?y.#ue(this.#T,e,this.#n,this.#i):{maxBlockDifference:1/0,totalDifference:1/0};this.#a.push(i);const s=this.#x===t,r=s&&i.maxBlockDifference<this.#e;s&&!r&&(this.#x=null);const n=this.#x;this.#T=e.slice(),this.#s++;let u=this.#x;if(this.#s===y.CYCLE){let o=0,c=null;for(let h=1;h<this.#a.length;h++)(this.#a[h]?.maxBlockDifference??1/0)<(this.#a[o]?.maxBlockDifference??1/0)?(c=o,o=h):(c===null||(this.#a[h]?.maxBlockDifference??1/0)<(this.#a[c]?.maxBlockDifference??1/0))&&(c=h);this.#O=this.#a[o]?.maxBlockDifference??1/0,this.#z=c===null?1/0:this.#a[c]?.maxBlockDifference??1/0,u=(this.#a[o]?.maxBlockDifference??1/0)<this.#e?o:null,this.#x=u,this.#a=[],this.#s=0}return{cycleIndex:t,maxBlockDifference:i.maxBlockDifference,totalDifference:i.totalDifference,shouldDrop:r,dropIndex:n,nextDropIndex:u,lowestCycleDifference:this.#O,runnerUpCycleDifference:this.#z}}weave(e,t,i,s,r){if(s==="c")return t.slice();const n=t.slice(),u=s==="p"?e:i,o=n.length/this.#i,c=r?1:0;for(let h=c;h<this.#i;h+=2)n.set(u.subarray(h*o,(h+1)*o),h*o);return n}reset(){this.#s=0,this.#x=null,this.#a=[],this.#T=null,this.#O=1/0,this.#z=1/0}#w(e,t,i,s){const r=this.#n,n=this.#i,u=2-i,o=2-i,c=s[e],h=s[t],v=y.#fe(c,h,r,n,i);let l=0,a=0,p=0,d=0,E=0,g=0;for(let _=2;_<n-2;_+=2){const R=(_-2)/2,Q=u-1+R*2,$=u+1+R*2,K=u+3+R*2,G=u+R*2,X=G+2,U=o+R*2,w=U+2,ie=u+R*2;for(let S=8;S<r-8;S++){const P=(v[ie*r+S]??0)|(v[(ie+2)*r+S]??0);if(P===0)continue;const se=(s.c[Q*r+S]??0)+((s.c[$*r+S]??0)<<2)+(s.c[K*r+S]??0),N=Math.abs(3*((c[G*r+S]??0)+(c[X*r+S]??0))-se),B=Math.abs(3*((h[U*r+S]??0)+(h[w*r+S]??0))-se);N>23&&(P&1)!==0&&(l+=N),B>23&&(P&1)!==0&&(d+=B),N>42&&(P&2)!==0&&(a+=N),B>42&&(P&2)!==0&&(E+=B),N>42&&(P&4)!==0&&(p+=N),B>42&&(P&4)!==0&&(g+=B)}}a<500&&E<500&&(p>=500||g>=500)&&Math.max(p,g)>3*Math.min(p,g)&&(a=p,E=g);const x=Math.floor(l/6+.5),D=Math.floor(d/6+.5),b=Math.floor(a/6+.5),m=Math.floor(E/6+.5),z=Math.max(x,D)/Math.max(Math.min(x,D),1),W=Math.max(b,m)/Math.max(Math.min(b,m),1),H=Math.max(b,m)/Math.max(Math.max(x,D),1);return(b>=500||m>=500)&&(b*2<m||m*2<b)||(b>=1e3||m>=1e3)&&(b*3<m*2||m*3<b*2)||(b>=2e3||m>=2e3)&&(b*5<m*4||m*5<b*4)||(b>=4e3||m>=4e3)&&W>z||H>.005&&Math.max(b,m)>150&&(b*2<m||m*2<b)?b>m?t:e:x>D?t:e}static#fe(e,t,i,s,r){const n=Array.from({length:Math.ceil(s/2)},()=>new Uint8Array(i)),u=r===1?1:0;for(let h=0;h<n.length;h++){const v=Math.min(s-1,u+h*2),l=n[h];if(l)for(let a=0;a<i;a++)l[a]=Math.abs((e[v*i+a]??0)-(t[v*i+a]??0))}const o=new Uint8Array(i*s),c=r===1?3:2;for(let h=1;h<n.length-1;h++){const v=c+(h-1)*2;if(v>=s)break;const l=n[h];if(l)for(let a=1;a<i-1;a++){const p=l[a]??0;if(p<=3)continue;let d=0;for(let m=a-1;m<=a+1;m++)d+=(n[h-1]?.[m]??0)>3?1:0,d+=(n[h]?.[m]??0)>3?1:0,d+=(n[h+1]?.[m]??0)>3?1:0;if(d<=1)continue;const E=v*i+a;if(o[E]=1,p<=19)continue;d=0;let g=!1,x=!1;for(let m=a-1;m<=a+1;m++)(n[h-1]?.[m]??0)>19&&(d++,g=!0),(n[h]?.[m]??0)>19&&d++,(n[h+1]?.[m]??0)>19&&(d++,x=!0);if(d<=3)continue;if(g&&x){o[E]|=2;continue}let D=!1,b=!1;for(let m=Math.max(a-4,0);m<Math.min(a+5,i);m++)h!==1&&(n[h-2]?.[m]??0)>19&&(D=!0),(n[h-1]?.[m]??0)>19&&(g=!0),(n[h+1]?.[m]??0)>19&&(x=!0),h!==n.length-2&&(n[h+2]?.[m]??0)>19&&(b=!0);g&&(x||D)||x&&(g||b)?o[E]|=2:d>5&&(o[E]|=4)}}return o}static#L(e,t,i){const s=new Uint8Array(t*i);for(let n=0;n<i;n++){const u=n*t,o=Math.max(0,Math.min(i-1,n===0?1:n-1))*t,c=Math.max(0,Math.min(i-1,n===i-1?i-2:n+1))*t,h=Math.max(0,Math.min(i-1,n<2?n===0?2:3:n-2))*t,v=Math.max(0,Math.min(i-1,n+2>=i?n===i-1?i-3:i-4:n+2))*t;for(let l=0;l<t;l++){const a=e[u+l]??0,p=e[o+l]??0,d=e[c+l]??0,E=e[h+l]??0,g=e[v+l]??0;(n===0?Math.abs(a-d)>y.COMB_THRESHOLD:n===i-1?Math.abs(a-p)>y.COMB_THRESHOLD:Math.abs(a-p)>y.COMB_THRESHOLD&&Math.abs(a-d)>y.COMB_THRESHOLD)&&Math.abs(4*a-3*(p+d)+E+g)>y.COMB_THRESHOLD*6&&(s[n*t+l]=255)}}let r=0;for(const n of[0,8])for(const u of[0,8])for(let o=n;o<i;o+=16)for(let c=u;c<t;c+=16){let h=0;for(let v=Math.max(1,o);v<Math.min(i-1,o+16);v++)for(let l=c;l<Math.min(t,c+16);l++){const a=v*t+l;s[a-t]===255&&s[a]===255&&s[a+t]===255&&h++}r=Math.max(r,h)}return r}static#ue(e,t,i,s){const r=y.DECIMATE_BLOCK/2,n=Math.ceil(i/r),u=Math.ceil(s/r),o=new Float64Array(n*u),c=e.length/(i*s);for(let l=0;l<s;l++){const a=Math.floor(l/r);for(let p=0;p<i;p++){const d=Math.floor(p/r),E=a*n+d,g=(l*i+p)*c;if(c===1){o[E]=(o[E]??0)+Math.abs((e[g]??0)-(t[g]??0));continue}const x=Math.round((e[g]??0)*.2126+(e[g+1]??0)*.7152+(e[g+2]??0)*.0722),D=Math.round((t[g]??0)*.2126+(t[g+1]??0)*.7152+(t[g+2]??0)*.0722);if(o[E]=(o[E]??0)+Math.abs(x-D),(p&1)!==0||(l&1)!==0)continue;let b=0,m=0,z=0,W=0,H=0,_=0,R=0;for(let X=l;X<Math.min(l+2,s);X++)for(let U=p;U<Math.min(p+2,i);U++){const w=(X*i+U)*c;b+=e[w]??0,m+=e[w+1]??0,z+=e[w+2]??0,W+=t[w]??0,H+=t[w+1]??0,_+=t[w+2]??0,R++}const Q=Math.round((-.114572*b-.385428*m+.5*z)/R),$=Math.round((-.114572*W-.385428*H+.5*_)/R),K=Math.round((.5*b-.454153*m-.045847*z)/R),G=Math.round((.5*W-.454153*H-.045847*_)/R);o[E]=(o[E]??0)+Math.abs(Q-$)+Math.abs(K-G)}}let h=-1;for(let l=0;l<u-1;l++)for(let a=0;a<n-1;a++)h=Math.max(h,(o[l*n+a]??0)+(o[l*n+a+1]??0)+(o[(l+1)*n+a]??0)+(o[(l+1)*n+a+1]??0));let v=0;for(const l of o)v+=l;return{maxBlockDifference:h,totalDifference:v}}}let le=null;const ce=.5,T=3,q=5,L=q+1,J=1e3,V=4,j=200,fe=.25,ue=1e3/60,de=.02,me=250,pe=1e3/30;function Z(f){if(!Number.isFinite(f)||f<0)throw new RangeError("filmCombThreshold must be a finite number greater than or equal to 0");return f}const ve=`#version 300 es
void main() {
