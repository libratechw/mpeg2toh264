...
- 検査用クリップは`packages/yadif/src/probe-clip.ts`にdata URLとして格納しています。1440x1080インターレース（TFF）の6フレームで、サイズは3,636バイト（base64で4.8 KB）です。2つのフィールドは明暗が反転し、フレームごとに入れ替わります。再生成用のffmpegコマンドはファイルの先頭に記載しています。
