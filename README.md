...
色差だけは例外で、MPEG-2 4:2:0の色差は1個の8×8 DCTなもののH.264は4個の4×4整数変換と2×2 DC Hadamard変換を使います。係数同士を一対一に対応させられないため、色差ブロックは8×8 IDCTで一度空間領域へ戻しH.264の4×4基底へ投影します。これによる丸め誤差の蓄積を抑えるため、色差のQPには-6のオフセットを使います。
...
`crates/mpeg2toh264-wasm`は`wasm-bindgen`ラッパーです。`packages/player`はMSEプレイヤー、`packages/yadif`はインターレース映像用のWebGLデインターレーサーです。

Dockerでは選択したローカルファイルをブラウザー内で変換するデモを起動できます。
...
別のHTTPサーバーにあるTSをCORS設定なしで読む場合はnginxの転送先を指定できます。
...
デモへ`/stream/api/channels/GR/27/stream`を指定するとnginxが`http://192.168.1.3:40772/api/channels/GR/27/stream`へ転送します。
...
デモでは`aribb24.js`へ渡して字幕・文字スーパーを利用して字幕と文字スーパーを表示しています。
